...
describe("Crowdfunding tests", function() {
    ...
    it("Should donate to campaign", async function() {
        const Campaign = await ethers.getContractFactory("CrowdFunding")
        const campaign = await Campaign.deploy()
        await campaign.deployed()
        ...
        await campaign.connect(addr2).donateToCampaign(0, {value: amount})
        ...
        // console.log('campaignDetails: ', parsedCampaigns)
        expect(parsedCampaigns[0]["amountCollected"]).to.equal('0.001')
    })
    //test for getDonators 
    it("Should get donators", async function() {
        const Campaign = await ethers.getContractFactory("CrowdFunding")
        const campaign = await Campaign.deploy()
        await campaign.deployed()
        ...
        const campaignDonators = await campaign.getDonators(0)
        console.log("Campaign donators: ", campaignDonators)
    })
})